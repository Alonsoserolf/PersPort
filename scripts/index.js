...
//detects mobile browsers to disable header parallax on phones
window.mobilecheck = () => {
  var check = false;
...
$(()=> {
  ...
  $(document).scroll(function() {
    //nav not fixed  
    if (!(body.hasClass('open')) && win.scrollTop() <= (cnt.offset().top)) {
      logo.removeClass('visible');
      mn.removeClass(fixed);
    ...
    if(!window.mobilecheck()){
      $('#para').each(function(){
        ...
        let elBackgrounPos = "center " + (y * speed) + "px";  
        $this.css('background-position', elBackgrounPos);
      });  
    }
      //parallax floating name
